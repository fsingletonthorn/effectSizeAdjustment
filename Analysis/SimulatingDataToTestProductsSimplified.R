source("Data/data_collection_cleaning.R")
...
nSim <- 121 # must be divisible by 121 for this to work
...
reductionSignifcantR <- simData %>% 
  ...
  summarise(meanPropChange = mean((fisherZDiffSim)/fis.o, na.rm = TRUE), mean.r = mean(fis.r, na.rm = T) , mean.o = mean(fis.o, na.rm = T), 
            ...
            medianPropChange = median((fisherZDiffSim)/fis.o, na.rm = TRUE), median.r = median(fis.r, na.rm = T) , median.o = median(fis.o, na.rm = T) ,
            medianDiff = median(fisherZDiffSim, na.rm = T),
            nTrue = sum(!is.na(significantSameDirection.r)), nValid = sum(!is.na(simData$significantSameDirection.r)))
# equiv test
reductionEquiv <- simData %>% 
  ...
  summarise(meanPropChange = mean((fisherZDiffSim)/fis.o, na.rm = TRUE), mean.r = mean(fis.r, na.rm = T) , mean.o = mean(fis.o, na.rm = T) ,
            ...
            medianPropChange = median((fisherZDiffSim)/fis.o, na.rm = TRUE), median.r = median(fis.r, na.rm = T) , median.o = median(fis.o, na.rm = T) ,
            medianDiff = median(fisherZDiffSim, na.rm = T),
            nTrue = sum(!is.na(simStatisticallyEquiv.ro)), nValid = sum(!is.na(simData$simStatisticallyEquiv.ro)))

# Bringing all of the above together:
tableReductionsSim <- rbind(Overall = reductionsimData, StatisticalSignificance = reductionSignifcantR, Nonequivalence = reductionEquiv)
...
modResSim2 <- data.frame(modelN = REModNonequiv$k, modelEstimate = REModNonequiv$b, MLM95lb = REModNonequiv$ci.lb, MLM95ub = REModNonequiv$ci.ub, row.names = "Nonequivalence")

modSumariesSim <- rbind(modResSim, modResSim1, modResSim)
...
accuracy$StatisticalSig[i] <- mean(simData$significantSameDirection.r == (simData$fis.r > 0), na.rm = T)
...
# write.csv(tableAllEstimatesSim, file = "Data/SimulationModelOutputSimp.csv", row.names = FALSE)
