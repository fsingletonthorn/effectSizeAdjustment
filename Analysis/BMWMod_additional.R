...
orgEffect_FT[i] ~ dnorm(trueOrgEffect[i], orgTau[i]) #  + mu_source_ori[source[i]]+ mu_study_ori[study[i]] 
repEffect_FT[i] ~ dnorm(trueRepEffect[i], repTau[i]) # + mu_study_rep[study[i]] 
...
mu[i] <- (alpha * (trueOrgEffect[i]) * equals(clust[i], 1)) + (mu_source_rep[source[i]])
