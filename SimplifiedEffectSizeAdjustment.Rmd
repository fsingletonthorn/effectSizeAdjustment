...
This analysis leads to similar conclusions to the models with exclusions, with an estimated effect size decease of r = `r ztor( REMod.p.val.cleaned$b[1] )`, 95% CI [`r ztor( REMod.p.val.cleaned[['ci.lb']][1] )`, `r ztor( REMod.p.val.cleaned[['ci.ub']][1] )`]. This represents a decrease of `r round((REMod.p.val.cleaned$b[1]/ mean(allData$fis.o, na.rm = T))*100)`%  (95% CI [`r round((REMod.p.val.cleaned[['ci.lb']][1] / mean(allData$fis.o, na.rm = T))*100)`%, `r round((REMod.p.val.cleaned[['ci.ub']][1] / mean(allData$fis.o, na.rm = T))*100)`%]) of the average original effect size. The projects and article level variances are functionally identical to the model that does not include replication p values as a moderator.
