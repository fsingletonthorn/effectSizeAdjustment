...
The first method is to re-perform the meta-analysis excluding studies using two exclusion criteria; (a) excluding studies in which the replication study was not significant and (b) removing statistically equivalent studies. Because both of these methods function by removing small replication effects, no significance testing was performed on the difference between the model estimates estimated decreases after accounting for small or near-null effects. It is certain that, at a population level, both of these exclusion rules would lower the size of the observed effect size decrease.
...
$difference_{j} = \gamma_0  + \gamma_1Pvalue  + u_{id} + u_{article} + u_{project} + e_{j}$

This model acts as a convenient method of allowing the model to fit the effect size decreases in cases in which the effect in the replication study did not produce low p values, p values more likely under the alternative than null hypothesis, making $\gamma_0$ an estimate of the difference between original and replication effects given a replication p value of 0. 

This approach is limited as the distribution of p values is uniform under the null hypothesis, as the probability of obtaining low p values under the alternative hypothesis is determined by the (unknowable) statistical power of the replication study, and as interpretation becomes difficult as we begin to involve hypothetical replication studies and p values that are impossible (i.e., p values of 0). In part to avoid the latter issue (i.e., getting a predicted score at an impossible p value), and in part as sensitivity analyses, this analysis was reperfored using logit transformed p values, and examining the estimated amount of effect size decrease at a replicaiton p value of the logit of the the smallest positive floating-point number possible using 32-bit integers and use IEC 60559 floating-point (double precision) arithmetic (2.220446e-16), returning values that are substantively identical to those reported below, see supplementary materials 4 for full model output.
...
These values represent changes  equivalent to a decrease of `r round( ( modRes1$modelEstimate / mean(allData$fis.o, na.rm = T))*100)` to `r round(( modRes2$modelEstimate / mean(allData$fis.o, na.rm = T))*100)`% of the average original effect size (a correlation coefficient of r = `r ztor(mean(allData$fis.o, na.rm = T))`). Looking at the range of the 95% confidence intervals for both of these subsamples, the coverage extends from a considerable decrease equivalent to `r round(( modRes2$MLM95lb  / mean(allData$fis.o, na.rm = T))*100)`% of the average original correlation, to a small increase in effect sizes equivalent to `r round(( modRes1$MLM95ub  / mean(allData$fis.o, na.rm = T))*100)`% of the average original effect size.
...
Including replication p values as a moderator in a mixed effects meta-analysis leads to similar conclusions to the models with exclusions, with an estimated effect size decease of r = `r ztor( REMod.p.val.cleaned$b[1] )`, 95% CI [`r ztor( REMod.p.val.cleaned[['ci.lb']][1] )`, `r ztor( REMod.p.val.cleaned[['ci.ub']][1] )`]. This represents a decrease of `r round((REMod.p.val.cleaned$b[1]/ mean(allData$fis.o, na.rm = T))*100)`%  (95% CI [`r round((REMod.p.val.cleaned[['ci.lb']][1] / mean(allData$fis.o, na.rm = T))*100)`%, `r round((REMod.p.val.cleaned[['ci.ub']][1] / mean(allData$fis.o, na.rm = T))*100)`%]) of the average original effect size. The project and article level variances are functionally identical to the model that does not include replication p values as a moderator. 
...
These results show that there was a substantial average decrease in effects sizes between the original and replication study and suggest that this is still the case even after accounting for the presence of non-zero effects. Across the whole sample, replication sample sizes fell by a mean of r = `r tableReductions["Overall", "Mean ES difference"]`, and were `r abs(round( tableReductions["Overall", "Mean proportion change"]*100))`% smaller in replication studies. The results of the multilevel meta-analysis results agree with this estimate, showing an estimated mean decrease of r = `r REModSum$estimate_Cor`, (95% CI [`r REModSum$CI95_Cor`]), equivalent to a `r REModSum$estimate_D` point Cohen's d decrease (95% CI [`r REModSum$CI95_d`]), or an estimated decrease of `r abs(round((REModSum$estimate_Z/mean(allData$fis.o, na.rm = T)),2))*100`%  (95% CI [`r abs(round((REModSum$CI95_Z/mean(allData$fis.o, na.rm = T))*100))[1]`%, `r abs(round((REModSum$CI95_Z/mean(allData$fis.o, na.rm = T))*100)[2])`%]) of the mean effect size in the original studies (a $Fisher_Z$ equal to r = `r ztor(mean(allData$fis.o, na.rm = T))`). All of these estimates are higher than that suggested by {Stanley, 2018 #1002} of 8 to 15%, possibly because of the different samples used (i.e., studies that were the focus of meta-analyses in Stanley et al. compared the various sampling strategies used in the large scale replication projects).
...
These results highlight some major issues in the psychological research literature. Looking at the raw average proportion decrease, -29%, or at the results of the multilevel model, an r =`r modSumariesR$modelEstimate[1]` (95% CI [`r REModSum$CI95_Cor`]) average decrease. Changes of this magnitude could make a considerable difference in many research scenarios, and researchers reading the literature should be aware that the effect sizes reported in the scientific literature are likely exaggerated.   Researchers should also be aware that their experiments are likely to be under-powered if they plan their sample sizes using the effect size reported in a previous experiment. As a heuristic for researchers performing formal sample size planning such as power analysis on the basis of previous research, researchers could follow the advice given in {Camerer, 2018 #967} and plan their experiments assuming that the true effect size is 50% of the reported size, a value matched by the more extreme 95% confidence interval of the estimated amount of effect size decrease using the multilevel meta-analytic framework. Alternatively, it may be preferable to rely on estimates of the smallest effect size of interest or use flexible analysis strategies that do not rely on precise a priori specification of the sample size to be collected {Schönbrodt, 2017 #672;Albers, 2018 #942;Lakens, 2014 #169} when practical constraints mean this approach is feasible. 
...
### SM4
#### Logit transformed p value multilevel model

`r knitr::kable(REMod.p.val.cleaned.logit.sum)`

Predicted output at examining the estimated amount of effect size decrease at a replicaiton p value of the logit of the the smallest positive floating-point number possible using 32-bit integers and use IEC 60559 floating-point (double precision) arithmetic (2.220446e-16): 

`r knitr::kable(predict(REMod.p.val.cleaned.logit, logit(.Machine$double.eps)))`
