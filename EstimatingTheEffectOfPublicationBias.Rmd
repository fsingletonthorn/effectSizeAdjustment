...
In using the results of this analysis to inform future research (e.g., for sample size planning) and to interpret the published literature, it is essential to take note of the level of heterogeneity in the amount of effect size attenuation across not just replicated effects but also across replication projects. The sampling decisions and replication methods used by each of the included projects appears to have had a considerable effect on the amount of effect size attenuation seen (e.g., looking at the results of Analysis 1, the estimated standard deviation of the  mean level of effect size attenuation  across projects is `r round(sqrt(REMod$sigma2[2]), 3)`, 95% CI [`r confIntsREMod[[1]]$random[2,c(2,3)]`]). The degree of heterogeneity in the amount of effect size attenuation across studies and projects means that using any single estimate of the amount of effect size decrease is likely to be misleading in the case of any individual replication study.

```{r, dpi=300}
catPlot 
```

Figure 3. A caterpillar plot of the effect size difference between original and replication study effect sizes ordered by magnitude, error bars are 95% confidence intervals around effect size differences.
