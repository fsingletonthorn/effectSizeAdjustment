...
Three versions of the Bayesian mixture model were developed. The main mixture model presented in text presents the model developed by Camerer et al,. (2018; see https://osf.io/xhj4d/ for their detailed description of this model). Two extensions of this model were developed and tested. All priors are chosen to be uninformative. The first extension allowed for the attenuation factor to differ by effect size, and the second added a third component, a component in which there was no effect size attenuation from the orignal to replication effect size. 

First, this is the reported model from {Camerer, 2018 #967}. This model has the issue of constraining all effect size attenuation factor factors to a single value, a restrictive assumption in the current case when different populations are being sampled from. It may be reasonable to assume that the degree of replication effect size attenuation differs by replication project. {Camerer, 2018 #967}'s model was adapted to allowed the amount of effect size attenuation to vary by replication project, as it is expected that these different projects will have different amounts of effect size attenuation, and as the differences are interesting in of themselves. The expanded model includes an alpha parameter, an effect size attenuation factor, for each included study. See Boxes SM1 to 3 for the JAGS code used in each of these analyses.
...
Box SM2. The altered model, estimating alpha independently for each replication project. 
