...
ADD IN BFs > 10 and Equviance test < .1
...
Because of the exploratory nature of the methods used to attempt to remove studies from this literature, a series of simulation studies were performed to ensure that these methods did not lead to spurious effects under reasonable assumptions. None of the methods of removing studies provides an unambigoulsly preferable outcome, although the frequentist methods (statistical significance, equivilence tests) clearly worked better as cut rules according to the simulation studies in the range of values which I consider plauible (i.e., between .25 and .75 for proportion of studies which are true nulls and for the degree of average effect size disattenutation which could be expected). None function particularly well when the true amount of effect size disattenuation is low. 
...
There is no default manner of LOO cross validation for MLM, so in order to assess whether the main results of this study are sensative to the inclusion of each of the replication projects and individual findnigs within each replication project, the models were rerun using leave one out cross validation, excluding both the individual replicaiton attempts and the replication projects one at a time. When leaving out individual studies the range of point estimates (i.e., the difference between the smallest and largest estimate of the difference between original and replicaiton studies) for each of the LOO cross validation models did not exceed more than a Fisher z sore of `r maxDiffLOOStudy`. When excluding one replication project at a time, model estimate ranges did not exceed `r maxDiffLOOProject`. See supplementary material [!] for a table of the proportion of model estimate p values below .05, and estimate quintiles for each model from the leave on out cross validation on the study and project levels. None of these changes would lead to substsantially different conclusions being drawn from the model output. 
...
In order to assess whether the methods that were used to estimate the proportion change in studies exluding null results develop reasonable estimates, a series of simulations were performed. Simulations took as a starting point the observed effects in the original studies, estimating a true effect from these original results based on the Fisher Transformed standard error (i.e., estimating the true effect of each original study assuming a normal distribution with a mean of the original effect and a standard deviation of the standard error), and applying an attentuation factor (i.e., the proportion by which the true effect is reduced between initial and replication studies). Simulations were performed on attenuation factors from 0 to 1 in steps of .1. Simulation studies also varied the number of true effects, also varying between 0 and 1 in stepts of .1, setting some studies to be 0 randomly. Notably, these simulations assumed that the probability of each study being a true null results was unrelated to the original effect size, an assumption that may not be reasonable in practice. The following is a table of how each method functions under each scenario, along with the number of simulations that make up each value.
